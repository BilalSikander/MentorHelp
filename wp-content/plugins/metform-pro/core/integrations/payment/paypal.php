...
class Paypal
{
	...
	public function init($args, $request)
	{
		...
		$first_name = isset($this->form_data[$this->payment_first_name[0]]) ? $this->form_data[$this->payment_first_name[0]] : '';
		$last_name = isset($this->form_data[$this->payment_last_name[0]]) ? $this->form_data[$this->payment_last_name[0]] : '';
		...
		if (in_array($type, ['success', 'cancel'])) {
			...
			error_log('Success page : '.$success_url. ' Cancel Page '.$cancel_url);
			$settings = \MetForm\Core\Admin\Base::instance()->get_settings_option();
			$page_id =  $settings['mf_thank_you_page'];
			$redirect_url = get_page_link($page_id) . '?id=' . $this->entry_id;
